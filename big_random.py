...
Description: Simulates simplified version of Plants versus Zombies in big 10x10 world with random planting policy.

This version takes place in a 10x10 world where the
...
def simulateGame():
    ...
    while True:
        ...
        if len(free) == 0:
            ...
            # action 100 corresponds to not planting aka placing 0
            action = index if state[index] == 3 else 100
        tup.append(action)
...
def main():
    data = generateData()
    data.to_csv("game_data_big.csv", index=False)
